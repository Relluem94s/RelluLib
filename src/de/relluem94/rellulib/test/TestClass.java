...
public class TestClass extends SplashScreen{
	...
	public static void eventExecutorTest(){
		...
		TestEvent testevent_2 = new TestEvent();
		TestEvent testevent_3 = new TestEvent();
		TestEvent testevent_4 = new TestEvent();
		TestEvent testevent_5 = new TestEvent();
		
		
		ee.registerEvent(testevent_0); //First Event
		ee.registerEvent(testevent_1); //Second Event (Double Reg)
		ee.registerEvent(testevent_2); //Next Event (Double Reg)
		ee.registerEvent(testevent_3); //Next Event (Double Reg)
		ee.registerEvent(testevent_4); //Next Event (Double Reg)
		ee.registerEvent(testevent_5); //Next Event (Double Reg)
		
		
		ee.removeEvent(testevent_3.getID()); // Event 3 Wird entfernt
		ee.removeEvent(testevent_5); // Event 5 wird entfernt
		
		ee.registerEvent(testevent_5); // Event 5 wird neu registriert und ist jetzt Event 3 da in Slot 3 wieder Platz ist
		
		ee.executeEvents(); // FÃ¼hrt alle Registrierten Events aus
		...
		LogUtils.log("Es wurde" + m1 + ee.getEventsAmount() + " Event" + m2 + "ausgeführt. Es sind noch " + ee.getEventsFreeAmount() + " von " 
